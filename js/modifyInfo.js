import { checkNickname } from '../apis/signupRequest.js';
...
import { userModify, userDelete } from '../apis/modifyInfoRequest.js';
...
const deleteAccount = async () => {
    const callback = async () => {
        ...
        if (status === HTTP_OK) {
            try {
                await requestJson(`${getServerUrl()}/api/auth/logout`, {
                    method: 'POST',
                    credentials: 'include',
                });
            } catch (error) {
